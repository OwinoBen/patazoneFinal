(function($){var NivoSlider=function(element,options){var settings=$.extend({},$.fn.nivoSlider.defaults,options);var vars={currentSlide:0,currentImage:'',totalSlides:0,running:false,paused:false,stop:false,controlNavEl:false};var slider=$(element);slider.data('nivo:vars',vars).addClass('nivoSlider');var check_windown=$(window).width();if($('.banner-nivo-slider-mobile-and-web').length>0&&check_windown<=767){$('.banner-nivo-slider-mobile-and-web').each(function(){var repalce_url=$(this);var new_url=repalce_url.attr('data-src-mobile');repalce_url.attr('src',new_url)});}
setTimeout(function(){if($('.banner-nivo-slider-mobile-and-web').length>0&&check_windown>767){$('.banner-nivo-slider-mobile-and-web').each(function(){var repalce_url=$(this);var new_url=repalce_url.attr('data-src-desktop');repalce_url.attr('src',new_url)});}},1000);var kids=slider.children();kids.each(function(){var child=$(this);var link='';if(!child.is('img')){if(child.is('a')){child.addClass('nivo-imageLink');link=child;}
...
var sliderImg=$('<img/>').addClass('nivo-main-image');if(check_windown<=767){sliderImg.attr('src',vars.currentImage.attr('data-src-mobile')).show();}else{sliderImg.attr('src',vars.currentImage.attr('data-src-desktop')).show();}
slider.append(sliderImg);$(window).resize(function(){slider.children('img').width(slider.width());sliderImg.attr('src',vars.currentImage.attr('src'));sliderImg.stop().height('auto');$('.nivo-slice').remove();$('.nivo-box').remove();});slider.append($('<div class="nivo-caption"></div>'));var processCaption=function(settings){var nivoCaption=$('.nivo-caption',slider);if(vars.currentImage.attr('title')!=''&&vars.currentImage.attr('title')!=undefined){var title=vars.currentImage.attr('title');if(title.substr(0,1)=='#')title=$(title).html();if(nivoCaption.css('display')=='block'){setTimeout(function(){nivoCaption.html(title);},settings.animSpeed);}else{nivoCaption.html(title);nivoCaption.stop().fadeIn(settings.animSpeed);}}else{nivoCaption.stop().fadeOut(settings.animSpeed);}}
